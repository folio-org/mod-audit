...
class UserFieldExclusionHandlerTest {
  ...
  void onSettingChanged_shouldExcludeFieldsAndDeleteEmptyRecords_whenValidListProvided() {
    when(userEventDao.excludeFieldsFromAll(any(), eq(conn), eq(TENANT_ID)))
    ...
    verify(userEventDao).excludeFieldsFromAll(
      eq(Set.of("personal.email", "barcode")), eq(conn), eq(TENANT_ID));
    verify(userEventDao).deleteEmptyUpdateRecords(conn, TENANT_ID);
  }

  @Test
  void onSettingChanged_shouldNotExclude_whenEmptyString() {
    var result = exclusionHandler.onSettingChanged("", conn, TENANT_ID);
  ...
  @Test
  void onSettingChanged_shouldNotExclude_whenEmptyJsonArray() {
    var result = exclusionHandler.onSettingChanged("[]", conn, TENANT_ID);
  ...
  @Test
  void onSettingChanged_shouldNotExclude_whenValueIsNull() {
    var result = exclusionHandler.onSettingChanged(null, conn, TENANT_ID);
  ...
  @Test
  void onSettingChanged_shouldNotExclude_whenMalformedJson() {
    var result = exclusionHandler.onSettingChanged("not-json", conn, TENANT_ID);
