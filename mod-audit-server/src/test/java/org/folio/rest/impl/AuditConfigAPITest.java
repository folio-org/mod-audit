...
public class AuditConfigAPITest extends ApiTestBase {
  ...
  void shouldUpdateSetting(String groupId, String key, int updatedValue, int pageSize, int retentionPeriod,
                           Headers headers) {
    ...
    given().headers(headers).get(AUDIT_CONFIG_SETTINGS_PATH.formatted(groupId))
      ...
      .body("totalRecords", equalTo(3))
      .body("settings[1].key", equalTo(RECORDS_PAGE_SIZE.getValue()))
      .body("settings[1].value", equalTo(pageSize))
      .body("settings[2].key", equalTo(RETENTION_PERIOD.getValue()))
      .body("settings[2].value", equalTo(retentionPeriod));
  }
