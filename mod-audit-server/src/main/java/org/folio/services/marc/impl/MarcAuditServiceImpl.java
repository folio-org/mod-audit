...
public class MarcAuditServiceImpl implements MarcAuditService {
  ...
  public Future<RowSet<Row>> saveMarcDomainEvent(SourceRecordDomainEvent event) {
    ...
    LOGGER.debug("saveOrderAuditEvent:: Trying to save SourceRecordDomainEvent tenantId: '{}', eventId: '{};", tenantId,
      event.getEventId());
    var record = event.getEventPayload().getNewRecord() == null ? event.getEventPayload().getOld()
  ...
  private Future<RowSet<Row>> save(SourceRecordDomainEvent event, String tenantId) {
    ...
    try {
      ...
      LOGGER.warn(
        "saveMarcBibDomainEvent:: Error during mapping SourceRecordDomainEvent to MarcAuditEntity for event '{}'",
        event.getEventId());
      return handleFailures(e, event.getEventId());
    }
    if (entity.diff() == null || entity.diff().isEmpty()) {
      LOGGER.debug("saveMarcBibDomainEvent:: No changes detected, skipping save record '{}' and tenantId='{}'",
