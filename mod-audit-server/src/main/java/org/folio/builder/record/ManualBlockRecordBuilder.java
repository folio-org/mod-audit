...
public class ManualBlockRecordBuilder implements LogRecordBuilder {
  ...
  public List<LogRecord> buildLogRecord(JsonObject event) {
    ...
    LogRecord manualBlockLogRecord = new LogRecord().withObject(LogRecord.Object.MANUAL_BLOCK)
      ...
      .withUserBarcode(getNestedStringProperty(manualBlockJson, REQUESTER, BARCODE))
      .withDate(new Date())
      .withDescription(new ManualBlockDescriptionBuilder().buildDescription(manualBlockJson))
      .withLinkToIds(new LinkToIds()
        .withUserId(JsonPropertyFetcher.getProperty(manualBlockJson, USER_ID)));
